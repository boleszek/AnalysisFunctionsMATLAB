function [meanmaxpwr, meanstdpwr, medianmaxpwr, medianstdpwr] = GetPeakPWR(dmat,fqr,no,nt,skipt,params,usebl,baseline_pwr)
...
meanmaxpwr = zeros(size(fqr,1),no); % number of fq-bands by no
meanstdpwr = zeros(size(fqr,1),no);
medianmaxpwr = zeros(size(fqr,1),no); % number of fq-bands by no
...
for ii = 1:no
    ...
    % first take max, then mean
     meanmaxpwr(ff,ii) = mean(max(pwr(f1ind:f2ind,:))/numel(f1ind:f2ind));
     meanstdpwr(ff,ii) = std(max(pwr(f1ind:f2ind,:))/numel(f1ind:f2ind));
     medianmaxpwr(ff,ii) = median(max(pwr(f1ind:f2ind,:))/numel(f1ind:f2ind));
     medianstdpwr(ff,ii) = mad(max(pwr(f1ind:f2ind,:))/numel(f1ind:f2ind),1);
